          ...
          <div style={{display:"grid",gap:14}}>
            <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:20,padding:"24px"}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                {PRESETS.map(p=>(<PresetCard key={p.id} p={p} selected={preset===p.id} onSelect={setPreset}/>))}
              </div>
            </div>
            {isCustom && (
              <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:20,padding:"24px"}}>
      ...
      <div style={{maxWidth:760,margin:"0 auto"}}>
        ...
        <div style={{marginTop:20,background:"#ffffff06",border:"1px solid #ffffff12",borderRadius:18,padding:"28px"}}>
          ...
          <div style={{background:"linear-gradient(135deg,#0057ff12,#7c3aed12)",border:"1px solid #0057ff25",borderRadius:14,padding:"20px"}}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:16}}><span style={{fontSize:16}}>🎟️</span><span style={{fontSize:13,fontWeight:800,color:"#fff"}}>Your milestone unlocks your room</span></div>
            <div style={{display:"grid",gap:8}}>
              {[{m:"$10K+",e:"🌐 Monthly Community Calls",color:"#4361ee"},{m:"$50K+",e:"📍 European City Meetups",color:"#00c48c"},{m:"$250K+",e:"🏛️ Nexus Annual Summit",color:"#f59e0b"},{m:"$1M+",e:"👑 The Founder's Circle",color:"#e11d48"}].map(ev=>(
...
function ContactModal({onClose}) {
  ...
  return (
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",backdropFilter:"blur(4px)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
      <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:20,padding:"32px 28px",width:"100%",maxWidth:420,boxShadow:"0 24px 80px rgba(0,0,0,0.2)"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:28}}>
          <div><div style={{fontSize:11,fontWeight:700,color:C.accent,textTransform:"uppercase",letterSpacing:1,marginBottom:4}}>Get in touch</div><h3 style={{margin:0,fontSize:22,fontWeight:900,color:C.ink}}>Contact Nexus</h3></div>
          <button onClick={onClose} style={{width:36,height:36,borderRadius:10,border:`1.5px solid ${C.border}`,background:"transparent",cursor:"pointer",fontSize:16,color:C.inkMid,fontFamily:"inherit"}}>✕</button>
        </div>
        ...
        <div style={{maxWidth:1000,margin:"0 auto",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:12}}>
          ...
          <div style={{display:"flex",gap:16}}>
            {["Privacy","Terms","Contact"].map(l=>(
              <span key={l} onClick={l==="Privacy"||l==="Terms"?()=>setShowPrivacy(true):l==="Contact"?()=>setShowContact(true):undefined} style={{fontSize:12,color:"#4b5563",cursor:"pointer"}}>{l}</span>
            ))}
          </div>
        </div>
      </footer>
